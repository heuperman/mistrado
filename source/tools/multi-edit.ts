...
class MultiEditServer {
			...
			for (const [i, edit_] of edits.entries()) {
				...
				if (edit.replaceAll) {
					...
					if (!modifiedContent.includes(edit.oldString)) {
						throw new Error(
							`Edit ${i + 1}: oldString not found in file content`,
						);
					}
	...
	private validateArgs(args: MultiEditArgs): asserts args is MultiEditArgs {
		...
		for (const [i, edit] of args.edits.entries()) {
			...
			if (typeof edit.oldString !== 'string') {
				throw new TypeError(
					`Invalid edit ${i + 1}: oldString must be a string`,
				);
			}

			if (typeof edit.newString !== 'string') {
				throw new TypeError(
					`Invalid edit ${i + 1}: newString must be a string`,
				);
			}

			if (
				edit.replaceAll !== undefined &&
